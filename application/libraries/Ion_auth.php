...
class Ion_auth
{
	 ...
	 * @param int|string|bool $id
	 *
	 * @return bool Quando o usuario é operador
	 * @author Moises Duarte
	 */
	public function is_operador($id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_operador');

		$operador_group = $this->config->item('operador_group', 'ion_auth');

		return $this->in_group($operador_group, $id);
	}
	 ...
	 * @author Moises Duarte
	 */
	public function is_cobrador($id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_cobrador');

		$cobrador_group = $this->config->item('cobrador_group', 'ion_auth');

		return $this->in_group($cobrador_group, $id);
	}
